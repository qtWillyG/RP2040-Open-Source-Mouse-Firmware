/*
 * RP2040 (Raspberry Pi Pico) — Custom USB HID Mouse with selectable identity
 *
 ...
 *
 * NOTE: The VID/PIDs below are placeholder / example "custom device" values.
 * Use your own. For real / published projects use a properly allocated USB
 * Vendor ID, or a project VID such as pid.codes for hobby work.
 */
...
// Which profile to apply on boot. Change this index, or read it from a switch.
...
void setup() {
  ...
  // On the RP2040 Arduino core, USB may already be up by the time setup() runs.
...
void loop() {
  // Proof-of-life: nudge the cursor in a small square every 2 seconds.
  if (usb_hid.ready()) {
